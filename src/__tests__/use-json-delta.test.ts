...
import { useJsonDelta } from "../react";
...
describe("useJsonDelta — status transitions", () => {
	it("starts idle", () => {
		const { result } = renderHook(() => useJsonDelta());
...
describe("useJsonDelta — data assembly", () => {
	it("assembles patches into data", () => {
		const { result } = renderHook(() => useJsonDelta<{ title: string }>());
		act(() => result.current.consume(patch("title", "Hello")));
	...
	it("assembles nested paths", () => {
		const { result } = renderHook(() =>
			useJsonDelta<{ cards: { term: string }[] }>(),
		);
		act(() => {
	...
	it("each change produces a new data reference for React diffing", () => {
		const { result } = renderHook(() => useJsonDelta<{ n: number }>());
		const refs: object[] = [];
...
describe("useJsonDelta — middleware", () => {
	it("applies middleware before reconstruction", () => {
		const { result } = renderHook(() =>
			useJsonDelta<{ title: string }>({
				middleware: [
					(patch, next) =>
	...
	it("can drop patches via middleware", () => {
		const { result } = renderHook(() =>
			useJsonDelta<{ title: string; secret: string }>({
	...
	it("can fan out a patch to mirror a field", () => {
		const { result } = renderHook(() =>
			useJsonDelta<{ cards: { term: string; originalTerm: string }[] }>({
				middleware: [
					(p, next) => {
						next(p);
...
describe("useJsonDelta — callbacks", () => {
	it("calls onChange with the latest partial state on each patch", () => {
		const onChange = vi.fn();
		const { result } = renderHook(() =>
			useJsonDelta<{ title: string }>({ onChange }),
		);
	...
	it("calls onComplete with final state", () => {
		...
		const { result } = renderHook(() =>
			useJsonDelta<{ title: string }>({ onComplete }),
		);
		act(() => {
			result.current.consume(patch("title", "Hello"));
			result.current.complete();
	...
	it("sets status to error when consume throws after complete", () => {
		const onError = vi.fn();
		const { result } = renderHook(() => useJsonDelta({ onError }));
		act(() => {
...
describe("useJsonDelta — reset and reuse", () => {
	...
	it("complete() is idempotent", () => {
		const onComplete = vi.fn();
		const { result } = renderHook(() => useJsonDelta({ onComplete }));
		act(() => {
			result.current.consume(patch("x", 1));
			result.current.complete();
...
describe("useJsonDelta — streaming simulation", () => {
